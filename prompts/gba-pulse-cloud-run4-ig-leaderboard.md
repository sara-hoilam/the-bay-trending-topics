...
1. Run the automated fetch and merge (do **not** hand-write snapshot JSON yourself):
...
This:
- Calls `scripts/fetch-ig-benchmark.mjs` (followers from Instagram profile API only)
...
- Use numbers from the script output; do not invent follower counts.
