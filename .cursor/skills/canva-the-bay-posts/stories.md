...
| Date | today’s date, e.g. `15 September 2026` | 60, 1836 | 549 × 41 |
| THE BAY wordmark | (image) | 812, 1836 | 208 × 31 |

**Title:** near-black serif, 2–4 lines. **Body:** dark serif. **Date:** small, bottom-left — **always today’s date (HKT)** unless the user specifies another. **Chip:** terracotta pill, **white lowercase** location. **Wordmark:** bottom-right, do not move.
...
Always replace the template date with **today’s date**, using Hong Kong time (HKT). Format: `D Month YYYY` with no leading zero on the day (`15 September 2026`, not `15 Sep 2026` or `02 Feb 2026`).

Do not keep sample dates from the template (`9 September 2026`, `02 Feb 2026`, or whatever is already in the box). Only use a different date if the user explicitly specifies one.

Do not move or restyle the date box (60, 1836).

## Copy limits (from the templates)

- Title: about 8–18 words. Break with `\n` to match 2–4 lines.
- Body: one tight dek, about 20–35 words.
- Date: today’s date (HKT) in `D Month YYYY`, unless the user specifies another date.
