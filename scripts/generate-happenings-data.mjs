#!/usr/bin/env node
/**
 * Refresh happenings-events.json from Lifestyle + Entrepreneur rows in
 * source-links-data.json. Fetch method and listing URLs come from each source's
 * happeningsFetch metadata (generated by generate-source-links-data.mjs from
 * happenings-fetch-config.mjs).
 *
 * Usage: node scripts/generate-happenings-data.mjs
 */
import fs from "fs";
import path from "path";
import { fileURLToPath } from "url";
import { hktDateStr } from "./hkt-date.mjs";
import { addDays, fetchEventsForSource } from "./happenings-fetch-handlers.mjs";
import { annotateHighlight } from "./happenings-highlight-score.mjs";
import { execFileSync } from "child_process";

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const root = path.join(__dirname, "..");
const outPath = path.join(root, "happenings-events.json");
const sourceLinksPath = path.join(root, "source-links-data.json");

const HAPPENINGS_CATEGORIES = new Set(["Lifestyle", "Entrepreneur"]);

function loadHappeningsSources() {
  if (!fs.existsSync(sourceLinksPath)) return [];
  const sl = JSON.parse(fs.readFileSync(sourceLinksPath, "utf8"));
  return (sl.sources ?? []).filter((s) => HAPPENINGS_CATEGORIES.has(s.category));
}

function normalizeKey(ev) {
  return `${ev.sourceDomain}|${ev.title.toLowerCase().replace(/[^a-z0-9\u4e00-\u9fff]+/g, " ").trim()}`;
}

function loadExisting() {
  if (!fs.existsSync(outPath)) return { events: [] };
  return JSON.parse(fs.readFileSync(outPath, "utf8"));
}

function prunePast(events, today) {
  const cutoff = addDays(today, -14);
  return events.filter((ev) => !ev.end || ev.end >= cutoff);
}

function groupByDomain(events) {
  const map = new Map();
  for (const ev of events) {
    const domain = ev.sourceDomain ?? "unknown";
    if (!map.has(domain)) map.set(domain, []);
    map.get(domain).push(ev);
  }
  return map;
}

function withSourceCategory(events, category) {
  return events.map((ev) => ({
    ...ev,
    sourceCategory: ev.sourceCategory || category,
  }));
}

async function main() {
  const today = hktDateStr();
  try {
    execFileSync(process.execPath, [path.join(__dirname, "fetch-ticketflap-events.mjs")], {
      stdio: "inherit",
      timeout: 90000,
    });
  } catch (err) {
    console.warn(`Ticketflap scrape skipped: ${err.message}`);
  }

  const sources = loadHappeningsSources();
  if (!sources.length) {
    console.error("No Lifestyle/Entrepreneur sources in source-links-data.json");
    process.exit(1);
  }

  const existing = loadExisting();
  const existingByDomain = groupByDomain(existing.events ?? []);

  const fetched = [];
  const kept = [];

  for (const src of sources) {
    const method = src.happeningsFetch?.method ?? "html";
    console.log(`Fetching ${src.domain} [${src.category}/${method}] (${src.url})`);
    let domainEvents = [];
    try {
      domainEvents = withSourceCategory(await fetchEventsForSource(src, today), src.category);
      console.log(`  ${domainEvents.length} events from ${src.domain}`);
    } catch (err) {
      console.warn(`  Skip ${src.domain}: ${err.message}`);
    }

    if (domainEvents.length > 0) {
      fetched.push(...domainEvents);
    } else {
      const fallback = withSourceCategory(
        prunePast(existingByDomain.get(src.domain) ?? [], today),
        src.category,
      );
      if (fallback.length) {
        console.log(`  Keeping ${fallback.length} cached event(s) for ${src.domain}`);
        kept.push(...fallback);
      } else {
        console.log(`  No events for ${src.domain}`);
      }
    }
  }

  const merged = new Map();
  for (const ev of kept) merged.set(normalizeKey(ev), ev);
  for (const ev of fetched) merged.set(normalizeKey(ev), ev);

  const events = [...merged.values()]
    .map(annotateHighlight)
    .sort((a, b) => a.start.localeCompare(b.start));

  const highlightedCount = events.filter((ev) => ev.highlighted).length;
  const lifestyleCount = sources.filter((s) => s.category === "Lifestyle").length;
  const entrepreneurCount = sources.filter((s) => s.category === "Entrepreneur").length;

  const payload = {
    generatedFrom: `Lifestyle (${lifestyleCount}) + Entrepreneur (${entrepreneurCount}) source links · source-links-data.json happeningsFetch`,
    updatedAt: today,
    highlightScoreThreshold: 24,
    highlightedCount,
    events,
  };

  fs.writeFileSync(outPath, JSON.stringify(payload, null, 2) + "\n");
  console.log(`Wrote ${outPath} (${events.length} events, updatedAt=${today})`);
}

main().catch((err) => {
  console.error(err);
  process.exit(1);
});
